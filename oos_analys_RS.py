...
#         plt.show()
    ...
    def cal_inv_backlog (self, filtered_theta, demand):
        filtered_theta['arrive_time'] = filtered_theta['variable_name'].apply(lambda x: int(x.split('[')[1].split(',')[0]))
        #print(filtered_theta['variable_name'])
        filtered_theta = filtered_theta.reset_index(drop = True)
        #print(filtered_theta['arrive_time'].index)
        for t in range(len(filtered_theta['arrive_time'])):
            if filtered_theta['variable_name'][t][-3:-1] == 's1':
                filtered_theta['arrive_time'][t] += 2 
            if filtered_theta['variable_name'][t][-3:-1] == 's2':
                filtered_theta['arrive_time'][t] += 2 
            if filtered_theta['variable_name'][t][-3:-1] == 's3':
                filtered_theta['arrive_time'][t] += 3
        #print(filtered_theta['arrive_time'])
        arrive_quantity = filtered_theta.groupby('arrive_time').sum()['value']
        #print(len(demand))
        inventory =  [0] * (len(demand)) # period
        backlog =  [0] * (len(demand))
        for i in arrive_quantity.index:
            if i >= len(demand):
                arrive_quantity = arrive_quantity.drop(i)
        for i in range(len(demand)):
            if i not in arrive_quantity.index:
                arrive_quantity[i] = 0.0
        arrive_quantity = arrive_quantity.sort_index()
        #print(arrive_quantity)
        for i in range(0, len(demand)):
            if i == 0 :
                temp = self.I_0 - self.B_0 + arrive_quantity.loc[i] - demand[i]
                if  temp > 0:
    ...
    def cal_out_of_sample(self, solution, oos_demands):
        df_oos_cost = []
        df_details= []
        ...
        #print(filtered_order_quantity)
        if len(oos_demands.shape) == 1:
            i = 0
            demand = oos_demands.values.tolist()
            price = self.prices
            ...
            inventory_cost_single, backlog_cost_single, df_detail= self.cal_inv_backlog(filtered_order_quantity, demand)
            #print(inventory_cost_single, backlog_cost_single)
            ...
            df_oos_cost.append([i, fixed_order_cost, purchase_cost, inventory_cost_single, backlog_cost_single,(fixed_order_cost + purchase_cost +
                                                                                                                inventory_cost_single + backlog_cost_single)])
        if len(oos_demands.shape) == 2:
